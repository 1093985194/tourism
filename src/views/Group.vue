  ...
  <div style="height: 100%;overflow-x:hidden;">
    <div class="panel">
      <div>
        ...
        <router-link class="btn btn-default" style="float:right;margin-top:10px;margin-right: 10px" to="groupSet">添加新团</router-link>
      </div>
    </div>
    <div>
      <br>
      ...
      <div class="list">
        <div class="table-bordered" style="background-color: white">
          <table class="table table-hover table-striped" width='100%' border='0' cellspacing='0' cellpadding='0' style='table-layout: fixed'>
            <thead>
            <tr>
              <th align="left" style="width: 300px">旅游团名称</th>
              <th align="left">日期</th>
              <th align="left">价格</th>
              <th align="left">余量</th>
              <th align="left">操作</th>
            ...
            <tr v-for="(item,index) in search(keywords).slice(x,y)">
              ...
              <td>
                <a v-on:click="getIndex(index)"><router-link to="groupSet">修改</router-link></a>/<a href="#" v-on:click="deleteInfo(index)">删除</a>
              </td>
            </tr>
            </tbody>
          </table>
...
<script>
  export default {
    ...
    // mounted () {
    //   this.$axios.get('http://10.210.96.229:8080/api/scenic')
    //     .then(response => {
